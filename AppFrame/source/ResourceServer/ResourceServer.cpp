...
std::unordered_map<std::string, int> ResourceServer::_mapMV1Model;
ResourceServer::MV1ModelMap ResourceServer::_mv1Models;
...
int ResourceServer::MV1LoadModel(const TCHAR* FileName) {
    // キーの検索
    auto itr = _mapMV1Model.find(FileName);
    if (itr != _mapMV1Model.end())
    {
        // キーがあった
        return itr->second;
    }
    ...
    // キーとデータをmapに登録
    _mapMV1Model[FileName] = model;

    return model;
}

void ResourceServer::LoadModels(const MV1ModelMap& mv1modelmap) {

    for (auto&& dgm : mv1modelmap) {
        auto& key = dgm.first;
        // キーを検索する
        auto it = _mv1Models.find(key);
        if (it != _mv1Models.end()) {
            continue;
        }
        // MV1Modelをコピーする
        auto dg = dgm.second;

        dg._handle = ::MV1LoadModel(dg._filename.c_str());
        // _mv1Modelsに追加する
        _mv1Models.emplace(key.c_str(), dg);
    }

}

int ResourceServer::GetHandles(const std::string& key) {

    auto it = _mv1Models.find(key);
    if (it == _mv1Models.end()) {
        return -1;
    }
    return MV1DuplicateModel(it->second._handle);
}
